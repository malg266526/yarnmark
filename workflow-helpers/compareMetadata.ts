...
const { baseMetadataFilepath, branchMetadataFilepath, out, ghTableMarkdown } = argv.option([
  {
...
const getComparedMetadata = ({ baseMetadataFilepath, branchMetadataFilepath, headers, changeTreshholdPercentage = 0.1 }: GetComparedMetadataProps) => {
  ...
  const emote = getPercentageDifference(baseTotalSize, branchTotalSize) > changeTreshholdPercentage ? RedExclamationMark : GreenCheckMark
...
const comparisonMetadata = getComparedMetadata({ baseMetadataFilepath, branchMetadataFilepath, headers: ['Master', 'Branch', 'Change', 'Change %'] });
