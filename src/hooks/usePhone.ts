...
const getWindowWidth = () =>
  window.innerWidth < window.outerWidth ? window.innerWidth : window.outerWidth;
...
export const usePhone = () => {
  ...
  const handler = useCallback(
    (newWidth: number) => setIsPhone(newWidth <= phoneScreenSize),
    [setIsPhone],
  );
...
export const useTablet = () => {
  ...
  const handler = useCallback(
    (newWidth: number) => setIsTablet(newWidth <= tabletScreenSize),
    [setIsTablet],
  );
